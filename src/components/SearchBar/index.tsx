...
import { COUNTRIESCURRENCY } from "@/lib/countriesWithCurrency"
    ...
    <div ref={ref}>
      <div className="mt-1 relative">
        <button
          ...
          } text-gray-400 relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm`}
        ...
        >
          <span className="truncate flex items-center">
            <img
              ...
              src={`https://purecatamphetamine.github.io/country-flag-icons/3x2/${selectedValue.isoAlpha2}.svg`}
              className={"inline mr-2 h-4 rounded-sm"}
            />
            <span className="font-normal truncate">
              {selectedValue.name} - {selectedValue.currency.code} (
              {selectedValue.currency.symbol})
            </span>
          </span>
          <span
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              ...
              >
                {COUNTRIESCURRENCY.filter((country) =>
                  ...
                  COUNTRIESCURRENCY.filter((country) =>
                    ...
                    return (
                      ...
                      >
                        <img
                          alt={`${value.isoAlpha2}`}
                          src={`https://purecatamphetamine.github.io/country-flag-icons/3x2/${value.isoAlpha2}.svg`}
                          className={"inline mr-2 h-4 rounded-sm"}
                        />

                        <span className="font-normal truncate">
                          {value.name} - {value.currency.code} (
                          {value.currency.symbol})
                        </span>

                        {value.isoAlpha2 === selectedValue.isoAlpha2 ? (
